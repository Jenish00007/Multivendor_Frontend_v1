...
const AdminSideBar = ({ openSidebar }) => {
  ...
  return (
    <div className={`${openSidebar ? 'block' : 'hidden'} md:block w-full h-screen bg-white shadow-xl sidebar-shadow overflow-y-scroll hide-scrollbar sticky top-0 left-0 z-10`}>
      <div className="p-6 border-b" style={{ background: '#645faa' }}>
        <div className="relative">
          <input
            ...
            className="w-full px-4 py-2 pl-10 text-gray-700 bg-white rounded-lg focus:outline-none focus:ring-2"
            style={{ '--tw-ring-color': '#645faa' }}
            onFocus={(e) => e.target.style.boxShadow = '0 0 0 2px rgba(100, 95, 170, 0.5)'}
      ...
      <div className="p-4">
        {filteredMenuItems.map((item) => (
          <div key={item.id} className="w-full flex items-center p-4 rounded-lg transition-all duration-300 group" style={{ '--hover-bg': 'rgba(100, 95, 170, 0.1)' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(100, 95, 170, 0.1)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
            <Link to={item.path} className="w-full flex items-center">
              <div className={`p-2 rounded-lg ${isActive(item.path) ? 'text-white' : 'bg-gray-100 text-gray-600'}`} style={isActive(item.path) ? { background: '#645faa' } : {}} onMouseEnter={(e) => !isActive(item.path) && (e.currentTarget.style.background = 'rgba(100, 95, 170, 0.2)')} onMouseLeave={(e) => !isActive(item.path) && (e.currentTarget.style.background = '#f3f4f6')}>
                {item.icon}
              </div>
              <h5
                ...
                style={isActive(item.path) ? { color: '#645faa' } : {}}
                onMouseEnter={(e) => !isActive(item.path) && (e.currentTarget.style.color = '#645faa')}
                onMouseLeave={(e) => !isActive(item.path) && (e.currentTarget.style.color = '#4b5563')}
              >
                {item.title}
              </h5>
            </Link>
